...
conn.on('ready', () => {
  console.log('✅ Connected to VPS! Pulling latest commits and rebuilding all services...');

  const cmd = [
    "cd /var/www/qution-softower || cd /var/www/niva-vupa-qution-spftower || cd /root/niva-vupa-qution-spftower",
    "git reset --hard HEAD",
  ...
  conn.exec(cmd, { pty: true }, (err, stream) => {
    ...
    stream.on('close', (code, signal) => {
      console.log(`\n\n🎉 Deployment successfully finished with exit code: ${code}`);
      conn.end();
    }).on('data', (data) => {
